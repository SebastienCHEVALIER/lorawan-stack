...
values will be stored in the Join Server.`,
	...
	endDevicesListQRCodeFormatsCommand = &cobra.Command{
		Use:     "list-qr-formats",
		Aliases: []string{"lsqrfmts", "lsqrfmt"},
  ...
  ttn-lw-cli end-devices list app1 \
    ...
    | ttn-lw-cli end-devices generate-qr`,
		RunE: asBulk(func(cmd *cobra.Command, args []string) error {
			var ids *ttnpb.EndDeviceIdentifiers
			if inputDecoder != nil {
				ids = new(ttnpb.EndDeviceIdentifiers)
				_, err := inputDecoder.Decode(ids)
				if err != nil {
					return err
				}
			...
			res, err := client.GenerateImage(ctx, &ttnpb.GenerateEndDeviceQRCodeImageRequest{
				FormatID:  formatID,
				EndDevice: *device,
				ImageSize: size,
			})
			...
			filename := path.Join(folder, fmt.Sprintf("%s.png", device.DeviceID))
			if err := ioutil.WriteFile(filename, res.Image, 0644); err != nil {
				return err
			}
...
func init() {
	...
	endDevicesGenerateQRCommand.Flags().String("format-id", "tr005draft2", "")
	endDevicesGenerateQRCommand.Flags().Uint32("size", 300, "")
